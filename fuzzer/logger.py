...
class FuzzLogger:
    ...
    BUGS_FIELDS = ["first_seen_iter", "first_seen_time", "category", "exc_type", "exc_msg", "input"]
    STATS_FIELDS = ["iteration", "elapsed_s", "runs_total", "bugs_unique", "corpus_size", "runs_per_sec"]
    ...
    def record(self, result: RunResult, corpus_size: int = 0) -> None:
        """Record a single run result."""
        self._iteration += 1
        now = time.monotonic() - self._start_time
        input_repr = result.input_data.decode("utf-8", errors="replace")[:120]
        ...
        if result.bug_type not in (BugType.NORMAL,):
            with open(self._tb_path, "a", encoding="utf-8") as f:
                f.write(f"\n{'='*60}\n")
                f.write(f"Iteration {self._iteration} | {result.bug_type.value} | {now:.1f}s\n")
